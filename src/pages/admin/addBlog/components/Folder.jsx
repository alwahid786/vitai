...
//                     }`}
//             >
//                 <div onClick={() => toggleFolder(folder.id, level)} className="flex gap-2 w-full p-2 bg-red- items-center">
//                     {/* Folder Icon */}
...
const Folder = ({
  ...
  // Updated check: compare using openFolders[level]?.id
  const isOpen = openFolders[level]?.id === folder.id;
  const [isEditing, setIsEditing] = useState(false);
  const [newName, setNewName] = useState(folder.name);
  const inputRef = useRef(null);
  const dispatch = useDispatch();
  ...
  const addArticlesHandlerLocal = (id) => {
    dispatch(setAddFolderData({ folderId: id, add: true }));
    dispatch(setDetailResponse(false));
  };
  ...
  return (
    <div className="relative">
      <div
        className={`flex justify-between items-center cursor-pointer rounded-lg pl-2 mb-2 ${isOpen ? "bg-gray-300 hover:bg-gray-400" : "bg-gray-200 hover:bg-gray-400"}`}
      >
        <div onClick={() => toggleFolder(folder.id, folder.name, level)} className="flex gap-2 w-full p-2  items-center">
          <span className="text-iconColor" style={{ color: iconColor }}>
            {isOpen ? (
              <FaRegFolderOpen className="text-iconColor" />
            ) : (
              <FaRegFolder className="text-iconColor" />
            ...
            <>
              {folder.subfolders?.map((subfolder) => (
                <Folder
                  ...
                  openFolders={openFolders}
                  toggleFolder={toggleFolder}
                  addArticlesHandler={addArticlesHandlerLocal}
                  level={level + 1}
                  activeContextMenu={activeContextMenu}
                  setActiveContextMenu={setActiveContextMenu}
                  contextMenuPosition={contextMenuPosition}
